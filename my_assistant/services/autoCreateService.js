...
async function autoCreateDependencies(task, hasExplicitTime = false) {
  try {
    ...
    if (task.dueDate) {
      ...
      try {
        // Check if user provided explicit time
        if (task.reminderTime) {
          reminderDate = new Date(task.reminderTime);
          if (isNaN(reminderDate.getTime())) {
            console.log(
              `  ⚠️ Invalid reminderTime provided, using default time`,
            );
            reminderDate = null;
          } else {
        ...
        // If no valid user time, use default (9:00 AM IST)
        if (!reminderDate) {
          ...
          if (typeof task.dueDate === "string") {
            ...
            throw new Error(
              `dueDate is neither string nor Date: ${typeof task.dueDate}`,
            );
          }
          ...
          if (dateParts.length === 3) {
            ...
            // Create date at 3:30 AM UTC (which equals 9:00 AM IST)
            reminderDate = new Date(Date.UTC(year, month, day, 3, 30, 0));
            ...
            console.log(
              `  ⏰ Default time (9:00 AM IST) set to UTC: ${reminderDate.toISOString()}`,
            );
            if (typeof formatISTDate === "function") {
              console.log(
                `  📍 Will show as IST: ${formatISTDate(reminderDate)}`,
              );
            }
          } else {
            throw new Error(`Invalid dueDate format: "${dueDateStr}"`);
          }
        }

        // Create reminder only if we have a valid date
        if (reminderDate && !isNaN(reminderDate.getTime())) {
          console.log(
            `  ⏰ Reminder will be set for: ${reminderDate.toISOString()}`,
          );
          ...
          console.log(
            `  ⚠️ No valid reminder date, skipping reminder creation`,
          );
        }
      } catch (error) {
        console.log(`  ❌ Error creating reminder: ${error.message}`);
        console.log(`  ⚠️ Skipping reminder creation for task: ${task.title}`);
      }
    } else {
...
async function updateDependencies(task, updates, previousValues = {}) {
  try {
    ...
    // ================================
    if (updates.dueDate) {
      // Use previousValues.oldDueDate instead of previousValues.oldDueDate
      ...
      if (task.reminderId) {
        ...
        if (reminder) {
          ...
          // Reminder ID exists but not found - create new one
          console.log(
            `  ⚠️ Reminder ID exists but reminder not found, creating new`,
          );
          const reminder = new Reminder({
            title: task.title,
          ...
          await task.save();
          console.log(`  ✅ New reminder created to replace missing one`);
        }
      }
      // CREATE REMINDER IF NOT EXISTS AND DUE DATE EXISTS
        ...
        task.reminderId = savedReminder._id;
        await task.save();
        console.log(`  ✅ New reminder created for due date`);
      }
    ...
    // ================================
    if (updates.title) {
      // Update linked reminder title
      if (task.reminderId) {
        const reminder = await Reminder.findById(task.reminderId);
        if (reminder) {
          reminder.title = task.title;
          await reminder.save();
          console.log(`  ✅ Reminder title updated to: "${task.title}"`);
        }
      }

      // Update linked note title
      if (task.noteId) {
        const note = await Note.findById(task.noteId);
        if (note) {
          note.title = task.title;
          await note.save();
          console.log(`  ✅ Note title updated to: "${task.title}"`);
        }
      }
    }
    ...
    // ================================
    if (updates.priority) {
      console.log(`  📌 Priority updated to: ${task.priority}`);

      // Update note content to reflect priority change
      if (task.noteId) {
        const note = await Note.findById(task.noteId);
        if (note && note.content) {
          // Check if note has priority information and update it
          if (note.content.includes("Priority:")) {
            note.content = note.content.replace(
              /Priority: \w+/,
              `Priority: ${task.priority}`,
            );
            await note.save();
            console.log(`  ✅ Note priority info updated`);
          } else {
            // Add priority to note
            note.content = `Priority: ${task.priority}\n\n${note.content}`;
            await note.save();
            console.log(`  ✅ Priority added to note`);
          }
        }
      }
    }
    ...
    if (updates.completed !== undefined) {
      ...
      // Handle reminder for completed tasks
      if (task.completed && task.reminderId) {
        const reminder = await Reminder.findById(task.reminderId);
        if (reminder) {
          console.log(`  ⏰ Reminder associated with completed task`);
          // Option: Delete reminder when task is completed (uncomment if desired)
          // await Reminder.findByIdAndDelete(task.reminderId);
          // task.reminderId = null;
          // await task.save();
          // console.log(`  🗑️ Reminder deleted due to task completion`);
        }
      }

      // Update note to reflect completion status
      if (task.noteId) {
        const note = await Note.findById(task.noteId);
        if (note && note.content) {
          if (task.completed && !note.content.includes("✓ COMPLETED")) {
            note.content = `✓ COMPLETED on ${formatISTDate(new Date())}\n\n${note.content}`;
            await note.save();
            console.log(`  ✅ Note marked as completed`);
          } else if (!task.completed && note.content.includes("✓ COMPLETED")) {
            note.content = note.content.replace(/✓ COMPLETED on .*\n\n/, "");
            await note.save();
            console.log(`  ✅ Note unmarked as completed`);
          }
        }
      }
    }
